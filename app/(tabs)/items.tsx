...
import React, { useEffect, useState } from 'react';
import { StyleSheet, Text, View, FlatList, TouchableOpacity, Image, Modal, Alert, Platform } from 'react-native';
import { Link, useNavigation, useRouter } from 'expo-router';
import { useItems } from '../items/itemsContext';
...
const Items = () => {
  ...
  const renderCategoryItem = ({ item }: { item: string }) => (
    <View style={styles.categoryItem}>
      {/* Category Image on the left */}
      <Image
        source={groupedItems[item].image ? { uri: groupedItems[item].image! } : { uri: 'https://via.placeholder.com/150' }} // Default image if no image available
        style={styles.categoryImage}
      />
  ...
  const showOptions = (categoryId: string) => {
    if (Platform.OS === 'web') {
    ...
    Alert.alert(
      ...
      '',
      [
        { text: 'Edit', onPress: () => handleEdit(categoryId) },
        { text: 'Delete', onPress: () => handleDelete(categoryId), style: 'destructive' },
  ...
  const openOptions = (categoryId: string) => {
    setSelectedCategoryId(categoryId);
    showOptions(categoryId);
  };
  
  const handleDelete = (categoryId: string) => {
    console.log('Deleted ');
    items.filter(item => item.categoryId === categoryId).forEach(item => deleteItem(item.id));
  };
  
  const router=useRouter();

  const handleEdit = (categoryId: string) => {
    // Alert.alert('Edit Log', `Editing log with id: ${categoryId}`);
    setModalVisible(false);
    router.push(`/items/editCategory?categoryId=${categoryId}`); // Navigate to the EditCategory screen
  };
  ...
  const LogOptionsModal = () => {
    return (
      <View style={styles.modalContainer}>
        ...
        <TouchableOpacity style={styles.modalButton} onPress={() => handleEdit(selectedId)}>
          <Text style={styles.modalButtonText}>Edit</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.modalButton} onPress={() => handleDelete(selectedId)}>
